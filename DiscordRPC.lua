...
assert(package.loadlib(mav_bat_path .. "lua53." .. extension,"*"))


local cffi = require("cffi")
local appId = "1142056624425533440"
...
local function unpackDiscordUser(request)
  return cffi.string(request.userId), cffi.string(request.username),
    cffi.string(request.discriminator), cffi.string(request.avatar)
end


local ready_proxy = cffi.cast("readyPtr", function(request)
  if discordRPC.ready then
    discordRPC.ready(unpackDiscordUser(request))
  end
end)

local disconnected_proxy = cffi.cast("disconnectedPtr", function(errorCode, message)
  if discordRPC.disconnected then
    discordRPC.disconnected(errorCode, ffi.string(message))
  end
end)

local errored_proxy = cffi.cast("erroredPtr", function(errorCode, message)
...
function discordRPC.initialize(applicationId, autoRegister, optionalSteamId)
  ...
  local eventHandlers = cffi.new("struct DiscordEventHandlers")
...
function discordRPC.updatePresence(presence)
  ...
  local cpresence = cffi.new("struct DiscordRichPresence")
...
function discordRPC.ready(userId, username, discriminator, avatar)
     reaper.ShowConsoleMsg("[discordrpc] Discord: ready (" .. userId .. ", " .. username .. ", " .. discriminator ", " .. avatar .. ")")
end

function discordRPC.disconnected(errorCode, message)
     reaper.ShowConsoleMsg("[discordrpc] Discord: disconnected (" .. errorCode .. ": " .. message .. ")")
end

function discordRPC.errored(errorCode, message)
     reaper.ShowConsoleMsg("[discordrpc] Discord: error (" .. errorCode .. ": " .. message .. ")")
end

discordRPC.initialize(appId, true)
--reaper.ShowConsoleMsg("Running RPC...")
local oldProjectName = " "


function update_rpc()
    if (reaper.GetProjectName("") ~= "") then
        local videoname = reaper.GetProjectName("");
        if (videoname ~= oldProjectName) then
            ...
            presence = {
                state = "Working in project",
                details = videoname,
                startTimestamp = now,
                largeImageKey = "reaper",
                smallImageKey = "",
                largeImageText = "Cockos REAPER DAW"
            }
            discordRPC.updatePresence(presence)
            oldProjectName = videoname
        end
    else
         if (oldProjectName ~= "") then
          ...
          discordRPC.updatePresence(presence)
          oldProjectName = reaper.GetProjectName("")
        end
    end
end
